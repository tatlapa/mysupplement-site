...
const selectedImage = ref<File | null>(null); // ✅ Stocke l'image
const imageError = ref<string | null>(null); // ✅ Stocke une éventuelle erreur

onMounted(async () => {
  await adminStore.getCategories();
});

// ✅ Schéma de validation (image supprimée car gérée autrement)
...
const submitProduct = handleSubmit(async (values) => {
  ...
  try {
    ...
    await adminStore.addProduct(formData); // 🔹 Assure-toi que ton API accepte `FormData`
